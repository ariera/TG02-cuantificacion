...
void setup() {
  ...
  size(2*img.width,img.height + 50 + TITLE_HEIGHT);
...
void displayTitle(){
  ...
  fill(color(0));
  text("Ejercicio 2: Cuantificacion" , 10, 20);
}

void displayInstructions(){
  textFont(createFont("Helvetica", 18));
  fill(color(0));
  text("Pulse '+' y '-' o CLICK para manejar el numero de niveles de gris\nESPACIO alterna entre Logaritmico y Lineal" , 10, img.height + 20 + TITLE_HEIGHT);
}

void displayImgDesc(){
  fill(color(0));
  rect(img.width, TITLE_HEIGHT, 140, 20);
  fill(color(255));
  text((intervalosLogaritmicos ? "logartimico " : "    lineal ") + niveles, img.width+10, 17 + TITLE_HEIGHT);  
}
